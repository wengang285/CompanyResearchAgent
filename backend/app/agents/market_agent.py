...
{{
    ...
    "market_position": {{
        ...
        "score": <根据品牌影响力、技术领先性、客户基础、护城河等因素综合评估，给出1-10分>,
...
## 市场地位评分计算规则（重要）：
市场地位评分 = (品牌影响力 × 0.3 + 技术领先性 × 0.3 + 客户基础 × 0.2 + 护城河强度 × 0.2)

评分标准（1-10分）：
- 品牌影响力：行业知名度、品牌价值（行业前三=8-10分，前五=6-7分，其他=3-5分）
- 技术领先性：技术壁垒、创新能力（领先=8-10分，中等=5-7分，落后=1-4分）
- 客户基础：客户粘性、市场份额（高=8-10分，中=5-7分，低=1-4分）
- 护城河强度：竞争壁垒、不可替代性（强=8-10分，中=5-7分，弱=1-4分）

请根据实际分析结果，严格按照上述规则计算市场地位评分，不要默认给7分。评分范围 1-10 分。只返回 JSON。"""
        else:
            # standard 模式
...
{{
    ...
    "market_position": {{
        ...
        "score": <根据品牌影响力、技术领先性、客户基础综合评估，给出1-10分>
    }},
    "swot": {{
        "strengths": ["优势1", "优势2"],
        "weaknesses": ["劣势1", "劣势2"],
...
## 市场地位评分计算规则（重要）：
请根据品牌影响力、技术领先性、客户基础等因素，综合评估市场地位，给出1-10分。

评分标准：
- 9-10分：行业领导者，品牌和技术都处于领先地位
- 7-8分：行业重要参与者，有一定竞争优势
- 5-6分：行业中等水平，竞争力一般
- 3-4分：行业较弱，竞争力不足
- 1-2分：行业边缘，竞争力很弱

请根据实际分析结果给出差异化评分，不要默认给7分。评分范围 1-10 分。只返回 JSON。"""
